...
class CourseDetailPageZh extends StatefulWidget {
  final String courseTitle;
  final List<DocumentSnapshot> lessonDocs;

  const CourseDetailPageZh({
    super.key,
  ...
  @override
  State<CourseDetailPageZh> createState() => _CourseDetailPageZhState();
}

class _CourseDetailPageZhState extends State<CourseDetailPageZh> {
  Map<String, Map<String, dynamic>> completedLessons = {};
  bool isLoading = true;

  //THEME MÀU TIẾNG TRUNG (GIỐNG LEARNING PAGE)
          ...
          SliverList(
            delegate: SliverChildBuilderDelegate(
              (context, index) {
                ...
                final bool canLearn = index == 0 ||
                    (() {
                      final prevId =
                          widget.lessonDocs[index - 1].id;
                      final prev = completedLessons[prevId];
                      return prev != null &&
                          prev["score"] == prev["total"];
                    })();

                final lessonData = widget.lessonDocs[index].data()
